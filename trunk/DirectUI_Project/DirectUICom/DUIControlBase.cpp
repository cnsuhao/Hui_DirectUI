...
STDMETHODIMP CDUIControlBase::OnLButtonDown(VARIANT_BOOL *pbResult)
{
...
STDMETHODIMP CDUIControlBase::CreateProp(ICtrlPluginProp* pPropParent,enumPropType eType,BSTR strPropName,BSTR strPropHelp,VARIANT_BOOL bIsStyle,IDUIPropBase **ppProp)
{
	...
	case _PROPTYPE_STRING:
		{
			::CoCreateInstance(__uuidof(StrProp),NULL,CLSCTX_INPROC_SERVER,
				__uuidof(IStrProp),(LPVOID*)&pProp);
		}
		break;
	case _PROPTYPE_TEXTSTYLE:
		{
			::CoCreateInstance(__uuidof(TextStyleProp),NULL,CLSCTX_INPROC_SERVER,
				__uuidof(ITextStyleProp),(LPVOID*)&pProp);
		}
		break;
	default:
		break;
